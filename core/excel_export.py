...
Stage cell marks/colors mirror ui.pages.data_view_page's
...
def _write_sum_data_sheet(ws: Worksheet, rows: list[dict[str, Any]]) -> None:
    ...
    stage defaults to "Open" -- see that module's docstring), plus a
    live % Complete column, rather than build_sum_data()'s own
    raw-file-derived status text.
    """
    headers = ["Index", "Description", "Approval Agency"] + _stage_headers() + ["VCR", "% Complete"]
    _write_header(ws, headers)
    ...
    for row_data in rows:
        ...
        pct = (done_count / len(required)) if required else None
        ...
        values += [_STAGE_MARKS.get(live_status.get(stage), "") for stage in range(1, STAGE_COUNT + 1)]
        values.append(row_data.get("vcr"))
        values.append(pct)
        ws.append(values)
        ...
        for stage, status in live_status.items():
            _write_stage_cell(ws, excel_row, stage, status)

        pct_col = STAGE_LAST_COL + 2
        if pct is not None:
            ws.cell(row=excel_row, column=pct_col).number_format = "0%"

    vcr_col = STAGE_LAST_COL + 1
    pct_col = vcr_col + 1
    _set_common_column_widths(ws, {vcr_col: 9, pct_col: 12})
    ws.freeze_panes = ws.cell(row=2, column=STAGE_FIRST_COL).coordinate
